...
const ServiceDetailPage = () => {
    ...
    if (authService.isAuthenticated()) {
      ...
      const fetchUserProfile = async () => {
        try {
          ...
          console.debug(
            "Could not fetch user profile for phone number:",
            error
          );
        }
      };
  ...
  const handleSubmitOrder = async () => {
    ...
    try {
          ...
          switch (field.type) {
            ...
            case "radio":
              return {
                ...
                option_id: value,
                value:
                  field.options?.find((opt) => opt.id === value)?.value ||
                  value.toString(),
              };
            case "checkbox":
              return Array.isArray(value) && value.length > 0
                ? {
                    ...
                    value: value
                      .map(
                        (v) =>
                          field.options?.find((opt) => opt.id === v)?.value || v
                      )
                      .join(", "),
                  }
                : null;
      ...
      if (hasFiles) {
        ...
        formData.append(
          "customer_email",
          customerInfo.email || "optional+email+notselected@buildingz.ae"
        );
        ...
        formattedFieldValues.forEach((fieldValue, index) => {
          formData.append(
            `field_values[${index}][field_id]`,
            fieldValue.field_id
          );
          if (fieldValue.option_id) {
            formData.append(
              `field_values[${index}][option_id]`,
        ...
        if (selectedAddress) {
          formData.append(
            ...
            selectedAddress.name || customerInfo.name
          );
          formData.append(
            "shipping_address[street]",
            selectedAddress.street || selectedAddress.address_line1
          );
  ...
  const renderField = (field) => {
    ...
    switch (field.type) {
        ...
        return (
          <div className="field-input-wrapper">
            <input
              ...
              value={value || ""}
              onChange={(e) =>
                handleFieldChange(field.id, e.target.value, "text")
              }
        ...
        return (
          <div className="field-input-wrapper">
            <textarea
              ...
              value={value || ""}
              onChange={(e) =>
                handleFieldChange(field.id, e.target.value, "textarea")
              }
              required={field.required}
              rows={4}
            />
        ...
        return (
          <div className="field-input-wrapper field-number-wrapper">
            <input
              ...
              value={value || ""}
              onChange={(e) =>
                handleFieldChange(field.id, e.target.value, "number")
              }
              min={field.min_value}
              max={field.max_value}
              step={field.step || 1}
        ...
        return (
          <div className="field-input-wrapper field-date-wrapper">
            ...
            <input
              ...
              value={value || ""}
              onChange={(e) =>
                handleFieldChange(field.id, e.target.value, "date")
              }
              required={field.required}
            />
          </div>
        ...
        return (
          <div className="field-input-wrapper field-time-wrapper">
            ...
            <input
              ...
              value={value || ""}
              onChange={(e) =>
                handleFieldChange(field.id, e.target.value, "time")
              }
              required={field.required}
            />
          </div>
            ...
            <div className="field-options-grid">
              {field.options?.map((option) => (
                <div
                  ...
                  onClick={() =>
                    handleFieldChange(field.id, option.id, "select")
                  }
                >
                  {option.image_url && (
        ...
        return (
          <div className="field-select-wrapper">
            <select
              ...
              value={value || ""}
              onChange={(e) =>
                handleFieldChange(field.id, parseInt(e.target.value), "select")
              }
              required={field.required}
            >
          ...
          <div className="field-options-grid">
            {field.options?.map((option) => (
              <div
                ...
                className={`option-card ${
                  value === option.id ? "selected" : ""
                } ${!option.image_url ? "option-card-compact" : ""}`}
      ...
      default:
        return (
          <p className="field-unsupported">نوع الحقل غير مدعوم: {field.type}</p>
        );
    }
  };
          ...
          {orderData?.order_id && (
            <div className="order-info">
              <p className="order-number">
                رقم الطلب: <strong>{orderData.order_id}</strong>
              </p>
            </div>
          )}

          {service?.whatsapp_numbers && service.whatsapp_numbers.length > 0 && (
            <div className="whatsapp-contact">
              <h3>تواصل معنا عبر واتساب</h3>
              <div className="whatsapp-numbers">
                {service.whatsapp_numbers.map((contact, index) => (
                  <a
                    key={index}
  ...
  const serviceSlug =
    ...
    (service?.name
      ...
      : "service");
  const pageTitle = `${service?.name || "خدمة"} - ${
    service?.category?.name || "خدمات"
  } - BuildingZ`;
  const pageDescription =
    service?.description ||
    `احجز خدمة ${service?.name || ""} على منصة BuildingZ`;
  const pageUrl = `${window.location.origin}/services2/${
    service?.id || "service"
  }-${serviceSlug}`;
  const serviceImage =
  ...
  return (
    <div className="service-detail-page">
      <Helmet>
        ...
        <meta
          name="keywords"
          content={`${service?.name}, ${service?.category?.name}, خدمات, BuildingZ, حجز خدمات`}
        />
              ...
              <section className="form-section">
                ...
                <div className="fields-container">
                  {service.fields.map((field) => (
                    <div
                      key={field.id}
                      className={`field-group field-type-${field.type}`}
                    >
        ...
        <div className="modal-overlay">
          <div className="modal-container modal-booking">
            ...
            <div className="booking-steps">
              ...
              <div
                className={`step ${bookingStep === "payment" ? "active" : ""}`}
              >
            ...
            {bookingStep === "details" && (
              <div className="modal-content">
                ...
                {/* Quick Summary - Service name only */}
                <div className="quick-summary-simple">
                  <p>
                ...
                <div className="form-grid">
                  ...
                  <div className="form-group full-width">
                    <label>
                      الإمارة <span className="required">*</span>
                    </label>
                    <select
                      className="form-input"
                  ...
                  <div className="form-group full-width">
                    <label>
                      التاريخ المفضل للخدمة <span className="required">*</span>
                    </label>
                    <input
                      type="date"
                      className="form-input"
