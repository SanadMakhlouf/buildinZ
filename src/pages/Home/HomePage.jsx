...
const HomePage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        try {
          const adsResponse = await fetch(`${config.BACKEND_URL}/api/advertisements/position/home_banner`);
          if (adsResponse.ok) {
            ...
            if (adsData.success && adsData.data && adsData.data.length > 0) {
              ...
              const adBanners = adsData.data.map((ad, index) => {
                const defaultBanner = defaultBanners[index % defaultBanners.length] || defaultBanners[0];
                return {
                  id: ad.id,
        ...
        try {
          const sidebarAdsResponse = await fetch(`${config.BACKEND_URL}/api/advertisements/position/sidebar`);
          if (sidebarAdsResponse.ok) {
            ...
            if (sidebarAdsData.success && sidebarAdsData.data && sidebarAdsData.data.length > 0) {
              // Use only images from ads, keep default titles/subtitles
              const adSideBanners = sidebarAdsData.data.slice(0, 2).map((ad, index) => {
                const defaultSideBanner = defaultSideBanners[index] || defaultSideBanners[0];
                return {
                  id: ad.id,
              ...
              console.log('Sidebar ad banners set (images only from ads):', adSideBanners);
            } else {
        ...
        try {
          const categoriesResponse = await fetch(`${config.BACKEND_URL}/api/categories`);
          if (categoriesResponse.ok) {
            ...
            if (responseData.success && responseData.data) {
              ...
              const mainCategories = responseData.data.filter(category => 
              ...
              console.log('Product categories set (main categories only):', sortedCategories);
            } else {
          ...
          try {
            ...
            console.log('Service categories response (fallback):', categoriesResponse);
            ...
            if (categoriesResponse?.success && categoriesResponse?.categories) {
              ...
              if (Array.isArray(categoriesResponse.data)) {
                ...
                categoriesData = Array.isArray(categoriesResponse.data.categories) 
        ...
        try {
          ...
          const sbMain = sbData.filter(cat => cat.parent_id === null || cat.parent_id === undefined);
        ...
        // Fetch products
        try {
          const productsResponse = await fetch(`${config.BACKEND_URL}/api/products`);
          if (productsResponse.ok) {
            const productsData = await productsResponse.json();
            console.log('Products response:', productsData);
            
            if (productsData.success && productsData.data?.products) {
              const formattedProducts = productsData.data.products.map(product => ({
                id: product.id,
                name: product.name,
                type: 'product',
  ...
  const getImageUrl = (imagePath, cacheBust = null) => {
    ...
    if (imagePath.startsWith('/storage/') || imagePath.startsWith('storage/')) {
      const fullUrl = config.utils.getImageUrl(imagePath.replace(/^\/?storage\//, ''));
      if (cacheBust) {
        const separator = fullUrl.includes('?') ? '&' : '?';
        return `${fullUrl}${separator}v=${cacheBust}`;
      }
              ...
              {sideBanners.map(banner => (
                ...
                >
                  <picture>
                    ...
                    <img src={banner.image} alt={banner.title || 'Side Banner'} />
                  </picture>
                </Link>
              ))}
            </div>
              ...
              {loading ? (
                ...
                categories.map(category => {
                  ...
                  const cacheBust = category.updated_at 
                    ...
                    : (category.created_at ? new Date(category.created_at).getTime() : null);
                  const imageUrl = categoryImage ? getImageUrl(categoryImage, cacheBust) : null;
                  // Use category id and updated_at for key to force re-render when image changes
                  const categoryKey = `${category.id}-${category.updated_at || category.created_at || ''}`;
                  return (
                    <Link 
                      key={categoryKey}
                      ...
                      <div className="noon-category-icon">
                        {imageUrl ? (
                          <img 
                            ...
                            alt={category.name}
                            loading="lazy"
                            style={{ display: 'block', opacity: 0, transition: 'opacity 0.3s' }}
                            onLoad={(e) => {
                            ...
                            onError={(e) => {
                              ...
                              if (!existingIcon || !existingIcon.classList.contains('fa-icon')) {
                                // Remove any existing non-FA icons
                                const nonFaIcons = parent.querySelectorAll('svg:not(.fa-icon)');
                                nonFaIcons.forEach(icon => icon.remove());
                              }
                            }}
                          />
                        ) : null}
                        {!imageUrl && <FontAwesomeIcon icon={getCategoryIcon(category.name)} />}
                      </div>
                    ...
                    <div className="noon-service-image">
                      {imageUrl ? (
                        ...
                        <div style={{ 
                          width: '100%', 
                          height: '100%', 
                          background: 'linear-gradient(135deg, #0A3259 0%, #1a4d80 100%)',
                          display: 'flex',
                          alignItems: 'center',
                      ...
                      <div className="noon-service-footer">
                        {product.originalPrice && product.price < product.originalPrice ? (
                          <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
                            <span className="noon-service-price" style={{ textDecoration: 'line-through', opacity: 0.6, fontSize: '0.85rem' }}>
        ...
        <div className="noon-section-container">
          ...
          <div className="noon-categories-grid">
            {categories.slice(0, 6).map(category => {
              const categoryImage = getCategoryImage(category);
              const imageUrl = categoryImage ? getImageUrl(categoryImage) : null;
              return (
                <Link 
                  key={category.id}
                  to={`/products?category=${category.id}`}
                ...
                >
                  ...
                  <div className="noon-category-card-content">
                    ...
                    <span className="noon-category-count">
                      {category.services_count || category.products_count || category.subcategories?.length || 0} {category.services_count ? 'خدمة' : 'منتج'}
                    </span>
                  </div>
                </Link>
              );
